...
def add_accessibility_banner(image_path, output_path):
    ...
    # Mesurer la taille du texte "Agrandissement Isolé"
    text_isolated_enlargement = "Agrandissement Isolé"
...
def process_child_image(image_path, xywh, new_image_path, zoom_factor):
    ...
    # Rogner l'image originale et la redimensionner
    ...
    # Créer un fond noir de la même taille que l'image rognée
    black_bg = Image.new('RGB', (cropped_width, cropped_height), (0, 0, 0))
    
    # Coller l'image rognée sur le fond noir
    black_bg.paste(img_cropped, (0, 0))
    
    # Enregistrer la nouvelle image
    black_bg.save(new_image_path)
    ...
    def process_children(parent_item):
        for child in parent_item.get("children", []):
            ...
            if any(keyword.lower() in child["title"].lower() for keyword in ["case", "vignette"]):
                ...
                new_image_path_abs, width, height = process_child_image(parent_image_path_abs, xywh, new_image_path_abs, zoom_factor=2)  # Changer le facteur de zoom comme souhaité
